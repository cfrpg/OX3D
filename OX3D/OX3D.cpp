...
int APIENTRY _tWinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPTSTR lpCmdLine,int nCmdShow)
{
	...
	LoadString(hInstance, IDC_OX3D, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_OX3D));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_OX3D);
...
LRESULT OnCommand(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	...
	{
		case IDM_ABOUT:
			MessageBoxA(hWnd,"About:\nOOXX 3D ver 1.0\nGame Design: WZH&LZY\ncfrpg@mail.nwpu.edu.cn","OX3D",MB_OK);
			break;
		case IDM_EXIT:
			DestroyWindow(hWnd);
			break;
		case IDM_NEW:
			...
			break;
		case ID_32773:
			MessageBoxW(hWnd,TEXT("说明：\n点击左侧方格放置棋子\n鼠标拖动右侧画面改变视角\n游戏规则：\n同一高度平面或竖直连成3个或体对角线连成3个即输"),TEXT("OX3D 帮助"),MB_OK);
			break;
		default:
